...
class RFResnet(nn.Module):
    def __init__(self, config=None, num_targets=None, pooling='avg', ff='conv'):
        super(RFResnet, self).__init__()
        assert config is not None, print("RFResnet config required!")
        self.config = config
        print("CONFIG:")
        print(self.config)
    ...
    @property
    def name(self):
        return f"RFResnet_{self.block_type}"
...
class RFResnetExplainable(RFResnet):
    def __init__(self, num_interpretable_concepts=None, num_targets=None, concept_layer_type='linear', **kwargs):
        super(RFResnetExplainable, self).__init__(config=config_cp_field_shallow_m2, num_targets=num_targets, ff=None, **kwargs)
